class KStrings {
  //generic texts
  static const String login = "Login";
  static const String signup = "Signup";
  static const String fullName = "Full name";
  static const String forgotPass = "Forgot Password";
  static const String submit = "Submit";
  static const String socialLogin = "Social login";
  static const String email = "Email";
  static const String mobile = "Mobile num";
  static const String pass = "Password";
  static const String passConfirm = "Confirm password";
  static const String getOTP = "Get OTP";
  static const String verify = "Verify";
  static const String sendOTP = "Send OTP";
  static const String model = "model";

  // welcome screen
  static const String welcTitle = "Welcome to OGTAXI.";
  static const String welcSubtitle = "Book taxis conviniently.";

  // login screen
  static const String loginTitle = "WELCOME BACK";
  static const String loginSubtitle = "Let's login to OGTAXI.";

  // signup screen
  static const String signupTitle = "Register to OGTAXI";
  static const String signupSubtitle = "Signup and enjoy all the benefits.";
}
